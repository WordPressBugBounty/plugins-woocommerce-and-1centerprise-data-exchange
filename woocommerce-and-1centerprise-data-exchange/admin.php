...
function wc1c_manage_edit_taxonomy_columns($columns) {
  $columns_after = array(
    'wc1c_guid' => __("1C Id", 'woocommerce-and-1centerprise-data-exchange'),
  );
...
function wc1c_woocommerce_attribute_taxonomies($attribute_taxonomies) {
  if (is_admin() && isset($_GET['page']) && $_GET['page'] == 'product_attributes') { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
    ...
    foreach ($attribute_taxonomies as $attribute_taxonomy) {
      ...
      if ($guid !== false) $attribute_taxonomy->attribute_label .= sprintf(" [%s: %s]", __("1C Id", 'woocommerce-and-1centerprise-data-exchange'), $guid);
    }
  }
...
function wc1c_plugin_action_links($actions) {
  $actions_before = array(
    // 'settings' => sprintf('<a href="%s" title="%s">%s</a>', admin_url("admin.php?page=woocommerce-1c"), esc_attr(__("View Settings", 'woocommerce-and-1centerprise-data-exchange')), __("Settings", 'woocommerce-and-1centerprise-data-exchange')),
    'donate' => sprintf('<a href="%s" title="%s" target="_blank">%s</a>', "https://money.yandex.ru/embed/donate.xml?account=410011766586472&quickpay=donate&payment-type-choice=on&default-sum=1000&targets=%D0%9F%D0%BB%D0%B0%D0%B3%D0%B8%D0%BD+%22%D0%9E%D0%B1%D0%BC%D0%B5%D0%BD+%D0%B4%D0%B0%D0%BD%D0%BD%D1%8B%D0%BC+%D0%BC%D0%B5%D0%B6%D0%B4%D1%83+WooCommerce+%D0%B8+1%D0%A1%3A%D0%9F%D1%80%D0%B5%D0%B4%D0%BF%D1%80%D0%B8%D1%8F%D1%82%D0%B8%D0%B5%D0%BC%22&target-visibility=on&project-name=&project-site=https%3A%2F%2Fwordpress.org%2Fplugins%2Fwoocommerce-and-1centerprise-data-exchange%2F&button-text=05&fio=on&mail=on&successURL=", esc_attr(__("Say thank you to plugin author", 'woocommerce-and-1centerprise-data-exchange')), __("Say thank you!", 'woocommerce-and-1centerprise-data-exchange')),
  );
...
function wc1c_admin_menu() {
  add_menu_page(__("1C", 'woocommerce-and-1centerprise-data-exchange'), __("1C", 'woocommerce-and-1centerprise-data-exchange'), 'manage_woocommerce', 'woocommerce-and-1centerprise-data-exchange', 'wc1c_admin_menu_page_settings', null, 100);
  add_submenu_page('woocommerce-and-1centerprise-data-exchange', __("Settings", 'woocommerce-and-1centerprise-data-exchange'), __("Settings", 'woocommerce-and-1centerprise-data-exchange'), 'manage_woocommerce', 'woocommerce-and-1centerprise-data-exchange');
  add_submenu_page('woocommerce-and-1centerprise-data-exchange', __("TODO", 'woocommerce-and-1centerprise-data-exchange'), __("TODO", 'woocommerce-and-1centerprise-data-exchange'), 'manage_woocommerce', 'woocommerce-1c-todo', 'wc1c_admin_menu_page_todo');
}
